import cv2
import numpy as np
import torch
from net_class import Net
import warnings
warnings.filterwarnings("ignore")

def apply_model(path):

    print("DISCLAIMER! DON'T USE THIS FOR REAL MEDICAL DEVICE")
    print("THIS IS JUST AN ML PROJECT")

    # 50x50 pixels
    img_size = 50

    img = cv2.imread(path, cv2.IMREAD_GRAYSCALE)

    img = cv2.resize(img, (img_size,img_size))

    img_array = np.array(img)

    img_array = img_array/255

    img_array = torch.tensor(img_array, dtype=torch.float32)

    net = Net()
    net = net.float()
    net.load_state_dict(torch.load("saved_model.pth"))
    net.eval()

    img_array = img_array.view(-1, 1, img_size, img_size)
    img_array = img_array.float()       # <-- critical
    
    net_out = net(img_array)[0]

    if net_out[0] >= net_out[1]:
        print()
        print()
        print("Prediction: Benign")
        print(f"Confidence: {round(float(net_out[0]), 3)}")
    else:
        print()
        print()
        print("Prediction: Melanoma")
        print(f"Confidence: {round(float(net_out[1]), 3)}")
        print()
        print()



